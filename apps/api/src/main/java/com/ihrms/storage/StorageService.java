...
/**
 * S3-compatible object storage (AWS SDK for Java v2) configured from {@code S3_*} — point at
 * local MinIO now, swap to AWS/R2 later by changing endpoint + keys. Files are reached only via
 * short-lived presigned URLs; raw storage keys/URLs are never returned to clients (§6). If
 * {@code S3_*} is unset the app still boots; the upload endpoints fail with 503 until configured.
 */
@Service
