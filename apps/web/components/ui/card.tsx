...
const cardVariants = cva('rounded-2xl border text-card-foreground', {
  variants: {
    variant: {
      ...
      // Soft-indigo brand surface (stat "moment" / hero panel). Ink text reads AA-safe on the light tint.
...
const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('flex flex-col space-y-1.5 p-4 sm:p-7', className)} {...props} />
  ),
);
...
const CardContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('p-4 pt-0 sm:p-7', className)} {...props} />
  ),
);
...
const CardFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('flex items-center p-4 pt-0 sm:p-7', className)} {...props} />
  ),
);
