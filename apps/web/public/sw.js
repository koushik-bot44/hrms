/*
 * hrorg.in Web Push service worker (Stage N2).
 * Push-ONLY: it shows OS notifications from 'push' events and routes 'notificationclick' — it does NOT
 * intercept fetch or cache anything, so hrorg.in is deliberately NOT turned into an offline/PWA app.
 ...
 *  - 'notificationclick' -> focus an open hrorg.in window, else open one at the payload url.
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'hrorg.in', {
      body: data.body || '',
      data: { url: data.url || '/' },
  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clients) => {
      for (const client of clients) {
        // Reuse an already-open hrorg.in window: focus it (and navigate where supported).
        if ('focus' in client) {
