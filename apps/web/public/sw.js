/*
 * IHRMS Web Push service worker (Stage N2).
 * Push-ONLY: it shows OS notifications from 'push' events and routes 'notificationclick' — it does NOT
 * intercept fetch or cache anything, so IHRMS is deliberately NOT turned into an offline/PWA app.
 ...
 *  - 'notificationclick' -> focus an open IHRMS window, else open one at the payload url.
  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clients) => {
      for (const client of clients) {
        // Reuse an already-open IHRMS window: focus it (and navigate where supported).
        if ('focus' in client) {
